...
const auditActorLabel = userId => userId ? `пользователь ${String(userId).slice(0, 8)}…` : 'автор не зафиксирован';
...
async function selectConcert(id) {
  ...
  const auditResult = await db.from('daria_audit_log').select('action,entity_type,entity_id,user_id,created_at').in('entity_id', relatedIds).order('created_at', { ascending: false }).limit(30);
