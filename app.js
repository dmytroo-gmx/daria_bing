...
const labels = {
  ...
  documentStatus: { NEW: 'НОВОЕ — ТРЕБУЕТ ПРОВЕРКИ', REVIEWED: 'ПРОВЕРЕНО', APPLIED: 'ПРИМЕНЕНО', REJECTED: 'ОТКЛОНЕНО' },
  negotiationStatus: { TO_BE_CLARIFIED: 'ТРЕБУЕТ УТОЧНЕНИЯ', NEGOTIATING: 'ПЕРЕГОВОРЫ', AGREED: 'СОГЛАСОВАНО', PAUSED: 'НА ПАУЗЕ', CLOSED: 'ЗАКРЫТО' },
...
const auditActorLabel = entry => entry.actor_name || (entry.user_id ? `пользователь ${String(entry.user_id).slice(0, 8)}…` : 'автор не зафиксирован');
function breakEvenTickets(concert, costs) {
  if (concert.break_even_mode !== 'CALCULATED') return concert.break_even_tickets == null ? null : Number(concert.break_even_tickets);
  const averageTicketPrice = Number(concert.average_ticket_price || 0);
  if (averageTicketPrice <= 0) return null;
  return Math.ceil(Math.max(0, Number(costs || 0)) / averageTicketPrice);
}
...
async function deleteRecord({ table, id, label: recordLabel, references = [], storagePath = '', refresh }) {
  ...
  const blocked = await blockingReferences(references);
  if (blocked.length) {
    const names = blocked.map(item => item.error ? 'не удалось проверить связанные данные' : `${item.label}: ${fmt(item.count)}`).join(' · ');
    setStatus(`Нельзя удалить «${recordLabel}»: сначала уберите связанные данные (${names}).`, true);
    return;
  }
  if (!window.confirm(`Удалить «${recordLabel}» навсегда? Это действие нельзя отменить.`)) return;
  const { error } = await db.from(table).delete().eq('id', id);
  if (error) { setStatus(`Не удалось удалить «${recordLabel}»: ${error.message}`, true); return; }
  if (storagePath) {
    const storage = await db.storage.from(documentBucket).remove([storagePath]);
    if (storage.error) { setStatus(`Запись «${recordLabel}» удалена, но файл нужно удалить из хранилища отдельно: ${storage.error.message}`, true); await refresh(); return; }
  }
  setStatus(`Удалено: ${recordLabel}`);
  await refresh();
}

const deleteConcert = concert => deleteRecord({ table: 'daria_concerts', id: concert.id, label: concert.event_name, references: [
...
const deleteOperator = operator => deleteRecord({ table: 'daria_ticketing_operators', id: operator.id, label: operator.name, references: [
  { table: 'daria_orders', column: 'operator_id', id: operator.id, label: 'заказы' }, { table: 'daria_tracking_links', column: 'operator_id', id: operator.id, label: 'ссылки' },
  { table: 'daria_daily_sales_snapshots', column: 'operator_id', id: operator.id, label: 'срезы продаж' }, { table: 'daria_unattributed_operator_reports', column: 'operator_id', id: operator.id, label: 'итоги без источника' }
...
function concertCard(concert, compact = false) {
  ...
  return `<article class="ops-concert ${state.selectedConcertId === concert.id ? 'selected' : ''}" data-concert-id="${esc(concert.id)}">
    ...
    <div class="concert-actions"><span class="risk ${riskClass(concert.risk_status)}">${esc(label('risk', concert.risk_status || 'GRAY'))}</span>${compact ? `<button class="text-button" type="button" data-action="details" data-id="${esc(concert.id)}">ОТКРЫТЬ</button>` : `<button class="text-button" type="button" data-action="details" data-id="${esc(concert.id)}">ДЕТАЛИ</button><button class="text-button" type="button" data-action="edit" data-id="${esc(concert.id)}">ИЗМЕНИТЬ</button><button class="text-button danger" type="button" data-delete-concert="${esc(concert.id)}">УДАЛИТЬ</button>`}</div>
  </article>`;
}
...
function renderConcertDetail() {
  ...
  if (state.detailTab === 'FINANCE') content = detail.expenses.map(expense => `<div class="detail-line"><b>${esc(expense.description || expense.category)}</b><span>${esc(label('expenseType', expense.expense_type))} · ${esc(label('paymentStatus', expense.payment_status))} · ${money(expense.amount, expense.currency)} · <button class="text-button danger" type="button" data-delete-detail-expense="${esc(expense.id)}">УДАЛИТЬ</button></span></div>`).join('') || '<div class="empty">Расходов ещё нет.</div>';
  if (state.detailTab === 'TRACKING') content = detail.links.map(link => `<div class="detail-line"><b>${esc(link.source_code)}</b><span>${esc(link.destination_url || link.statistical_url || 'Ссылка не задана')} · <button class="text-button danger" type="button" data-delete-detail-link="${esc(link.id)}">УДАЛИТЬ</button></span></div>`).join('') || '<div class="empty">Ссылок для отслеживания ещё нет.</div>';
  ...
  document.querySelectorAll('[data-delete-snapshot]').forEach(button => button.addEventListener('click', () => { const snapshot = detail.snapshots.find(item => item.id === button.dataset.deleteSnapshot); if (snapshot) deleteRecord({ table: 'daria_daily_sales_snapshots', id: snapshot.id, label: `срез ${dateLabel(snapshot.snapshot_date)}`, refresh: () => selectConcert(concert.id) }); }));
  document.querySelectorAll('[data-delete-detail-document]').forEach(button => button.addEventListener('click', () => { const document = detail.documents.find(item => item.id === button.dataset.deleteDetailDocument); if (document) deleteDocument(document, async () => { await Promise.all([loadDocumentsModule(), selectConcert(concert.id)]); }); }));
  document.querySelectorAll('[data-delete-detail-expense]').forEach(button => button.addEventListener('click', () => { const expense = detail.expenses.find(item => item.id === button.dataset.deleteDetailExpense); if (expense) deleteRecord({ table: 'daria_expenses', id: expense.id, label: expense.description || expense.category, refresh: async () => { await Promise.all([loadFinance(), loadOperations(), loadReportsModule(), selectConcert(concert.id)]); } }); }));
  document.querySelectorAll('[data-delete-detail-link]').forEach(button => button.addEventListener('click', () => { const link = detail.links.find(item => item.id === button.dataset.deleteDetailLink); if (link) deleteRecord({ table: 'daria_tracking_links', id: link.id, label: `ссылка ${link.source_code}`, refresh: async () => { await Promise.all([loadChannelsModule(), selectConcert(concert.id)]); } }); }));
...
function renderDocuments() {
  ...
  byId('document-list').innerHTML = documents.map(document => {
    ...
    return `<article class="expense-row"><div><small>${esc(documentTypeLabels[document.document_type] || document.document_type)} · ${esc(label('documentStatus', document.import_status))}</small><h3>${esc(document.source_name)}</h3><small>${esc(concert?.event_name || 'не привязано к концерту')} · ${esc(document.source_date || 'дата источника не указана')}</small></div><div class="expense-meta"><span>${esc(document.notes || 'без заметки')}</span><span>${document.created_at ? new Date(document.created_at).toLocaleString('ru-RU') : ''}</span></div><div class="document-actions">${preview}${apply}<button class="text-button" type="button" data-open-document="${esc(document.id)}">ОТКРЫТЬ</button><button class="text-button danger" type="button" data-delete-document="${esc(document.id)}">УДАЛИТЬ</button></div></article>`;
...
function renderChannels() {
  byId('channel-list').innerHTML = state.channels.map(channel => `<article class="ops-concert"><div><small>${esc(channel.code)}</small><h3>${esc(channel.name)}</h3><small>${channel.is_active ? 'АКТИВЕН' : 'НЕАКТИВЕН'}</small></div><div class="document-actions"><button class="text-button" type="button" data-edit-channel="${esc(channel.id)}">ИЗМЕНИТЬ</button><button class="text-button danger" type="button" data-delete-channel="${esc(channel.id)}">УДАЛИТЬ</button></div></article>`).join('') || '<div class="empty">Каналов ещё нет.</div>';
...
function renderFinance() {
  ...
  byId('expense-list').innerHTML = visible.map(expense => {
    ...
    return `<article class="expense-row"><div><small>${esc(concert ? `${concert.event_name} · ${concert.city}` : 'Концерт не найден')}</small><h3>${esc(expense.description)}</h3><small>${esc(expense.category)} · ${esc(label('expenseType', expense.expense_type))}${expense.expense_type === 'OPTIONAL_FUTURE' && expense.include_in_projected_cost ? ' · ВКЛЮЧЕНО В ПЛАН' : ''}${expense.supplier ? ` · ${esc(expense.supplier)}` : ''}</small></div><strong class="expense-amount">${money(expense.amount, expense.currency)}</strong><div class="expense-meta"><span class="expense-status ${expense.payment_status === 'PAID' ? 'paid' : ''}">${esc(label('paymentStatus', expense.payment_status))}</span><span>${expense.due_date ? `до ${esc(dateLabel(expense.due_date))}` : 'срок не указан'}</span></div><div class="document-actions"><button class="text-button" type="button" data-edit-expense="${esc(expense.id)}">ИЗМЕНИТЬ</button><button class="text-button danger" type="button" data-delete-expense="${esc(expense.id)}">УДАЛИТЬ</button></div></article>`;
...
function bindEvents() {
  ...
  byId('concerts-list').addEventListener('click', event => {
    const remove = event.target.closest('[data-delete-concert]');
    if (remove) { const concert = state.concerts.find(item => item.id === remove.dataset.deleteConcert); if (concert) deleteConcert(concert); return; }
  ...
  byId('order-list').addEventListener('click', event => {
    ...
    if (remove) { const order = state.orders.find(item => item.id === remove.dataset.deleteOrder); if (order) deleteRecord({ table: 'daria_orders', id: order.id, label: `заказ ${order.external_order_id}`, refresh: async () => { await Promise.all([loadSalesModule(), loadOperations(), loadReportsModule()]); } }); return; }
  ...
  byId('channel-list').addEventListener('click', event => {
    ...
    if (remove) { const channel = state.channels.find(item => item.id === remove.dataset.deleteChannel); if (channel) deleteChannel(channel); return; }
  ...
  byId('campaign-list').addEventListener('click', event => {
    ...
    if (remove) { const campaign = state.campaigns.find(item => item.id === remove.dataset.deleteCampaign); if (campaign) deleteCampaign(campaign); return; }
  ...
  byId('tracking-link-list').addEventListener('click', event => {
    ...
    if (remove) { const link = state.trackingLinks.find(item => item.id === remove.dataset.deleteLink); if (link) deleteRecord({ table: 'daria_tracking_links', id: link.id, label: `ссылка ${link.source_code}`, refresh: loadChannelsModule }); return; }
  ...
  byId('document-list').addEventListener('click', event => {
    ...
    if (remove) { const document = state.documents.find(item => item.id === remove.dataset.deleteDocument); if (document) deleteDocument(document); return; }
  ...
  byId('operator-list').addEventListener('click', event => {
    ...
    if (remove) { const operator = state.operators.find(item => item.id === remove.dataset.deleteOperator); if (operator) deleteOperator(operator); return; }
  ...
  byId('expense-list').addEventListener('click', event => {
    ...
    if (remove) { const expense = state.expenses.find(item => item.id === remove.dataset.deleteExpense); if (expense) deleteRecord({ table: 'daria_expenses', id: expense.id, label: expense.description || expense.category, refresh: async () => { await Promise.all([loadFinance(), loadOperations(), loadReportsModule()]); } }); return; }
