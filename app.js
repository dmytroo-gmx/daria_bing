...
function renderOperators() {
  byId('operator-list').innerHTML = state.operators.map(operator => `<article class="operator-card"><p class="eyebrow">${esc(operator.legacy_recommendation)}</p><h2>${esc(operator.name)}</h2><p><b>Комиссия:</b> ${known(operator.marketplace_commission, commission)} / ${known(operator.own_sales_commission, commission)}</p><p><b>Учёт переходов:</b> метка ${operator.supports_meta_pixel ? '✓' : '—'} · серверная передача ${operator.supports_capi ? '✓' : '—'} · диспетчер тегов ${operator.supports_gtm ? '✓' : '—'}</p><p><b>Данные покупателей:</b> ${esc(known(operator.customer_data_access))}</p><p><b>Выплата:</b> ${esc(known(operator.payout_timing))}</p><button class="text-button" type="button" data-edit-operator="${esc(operator.id)}">ИЗМЕНИТЬ</button></article>`).join('') || '<div class="empty">Операторов ещё нет.</div>';
  const rows = [
    ['Комиссия маркетплейса', operator => known(operator.marketplace_commission, commission)], ['Комиссия собственных продаж', operator => known(operator.own_sales_commission, commission)], ['Комиссия платежей', operator => known(operator.payment_provider_fee, commission)], ['Разовый запуск', operator => known(operator.setup_fee, value => money(value))], ['Ежемесячная плата', operator => known(operator.monthly_fee, value => money(value))], ['Данные покупателей', operator => known(operator.customer_data_access)], ['Срок выплаты', operator => known(operator.payout_timing)], ['Статистические ссылки', operator => operator.supports_statistical_links ? 'ЕСТЬ' : 'НЕ ЗАФИКСИРОВАНО'], ['Промокоды', operator => operator.supports_promo_codes ? 'ЕСТЬ' : 'НЕ ЗАФИКСИРОВАНО'], ['Рассылки', operator => operator.sms_marketing_available || operator.email_marketing_available ? `${operator.sms_marketing_available ? 'СМС' : ''}${operator.sms_marketing_available && operator.email_marketing_available ? ' / ' : ''}${operator.email_marketing_available ? 'ПОЧТА' : ''}` : 'НЕ ЗАФИКСИРОВАНО'], ['Эксклюзивность', operator => known(operator.exclusive_required)], ['Статус переговоров', operator => known(operator.negotiation_status)], ['Рекомендация Legacy', operator => known(operator.legacy_recommendation)]
  ];
...
function reportCard(concert, orders, expenses, campaigns) {
  ...
  return `<article class="report-card"><p class="eyebrow">${esc(label('status', concert.status))} · ${esc(label('risk', concert.risk_status))}</p><h2>${esc(concert.event_name)}</h2><small>${esc(concert.city)} · ${esc(dateLabel(concert.event_date))} · ${esc(concert.venue || 'площадка не указана')}</small><div class="report-grid"><div class="report-item"><span>ОПЛАЧЕННЫЕ БИЛЕТЫ</span><strong>${fmt(paidTickets)}</strong></div><div class="report-item"><span>ВЫРУЧКА ПО ОПЛАЧЕННЫМ</span><strong>${currencyTotals(paid, 'gross_revenue')}</strong></div><div class="report-item"><span>ФАКТИЧЕСКИЕ РАСХОДЫ</span><strong>${money(spend)}</strong></div><div class="report-item"><span>ОПЛАЧЕННЫЕ РАСХОДЫ</span><strong>${currencyTotals(paidExpenses)}</strong></div><div class="report-item"><span>ОБЯЗАТЕЛЬНО ОПЛАТИТЬ</span><strong>${currencyTotals(mandatory)}</strong></div><div class="report-item"><span>ОПЛАЧЕНО БЕЗ ИСТОЧНИКА</span><strong>${fmt(paid.filter(order => !order.campaign_id).reduce((sum, order) => sum + (Number(order.ticket_count) || 0), 0))}</strong></div><div class="report-item"><span>ЗАКАЗЫ ПО ДАННЫМ ПЛАТФОРМЫ</span><strong>${fmt(platformOrders)}</strong></div><div class="report-item"><span>КАМПАНИИ</span><strong>${campaigns.length}</strong></div></div><div class="report-meta"><div><b>СТОИМОСТЬ ЗАКАЗА ПО ПЛАТФОРМЕ</b>${ratio(spend, platformOrders, ' zł')}</div><div><b>СТОИМОСТЬ ПОДТВЕРЖДЁННОГО ЗАКАЗА</b>${ratio(spend, campaignOrders.reduce((sum, order) => sum + (Number(order.ticket_count) || 0), 0), ' zł')}</div><div><b>ОКУПАЕМОСТЬ ПО ПЛАТФОРМЕ</b>${ratio(platformValue, spend, '×')}</div></div></article>`;
}
