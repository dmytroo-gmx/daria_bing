...
function renderFinance() {
  ...
  byId('expense-list').innerHTML = visible.map(expense => {
    ...
    return `<article class="expense-row"><div><small>${esc(concert ? `${concert.event_name} · ${concert.city}` : 'Концерт не знайдено')}</small><h3>${esc(expense.description)}</h3><small>${esc(expense.category)} · ${esc(label('expenseType', expense.expense_type))}${expense.expense_type === 'OPTIONAL_FUTURE' && expense.include_in_projected_cost ? ' · ВКЛЮЧЕНО В ПЛАН' : ''}${expense.supplier ? ` · ${esc(expense.supplier)}` : ''}</small></div><strong class="expense-amount">${money(expense.amount, expense.currency)}</strong><div class="expense-meta"><span class="expense-status ${expense.payment_status === 'PAID' ? 'paid' : ''}">${esc(label('paymentStatus', expense.payment_status))}</span><span>${expense.due_date ? `до ${esc(dateLabel(expense.due_date))}` : 'строк не задано'}</span></div><button class="text-button" type="button" data-edit-expense="${esc(expense.id)}">РЕДАГУВАТИ</button></article>`;
